...
public class Craft implements Runnable {
		...
		try {
			while (!Display.isCloseRequested()) {
				...
				while (System.currentTimeMillis() >= lastTime + 1000L) {
					System.out.println(fps + " fps, " + Chunk.updates + " Chunks updated, totalUpdates:" + Chunk.totalUpdates + ", tickCount:" + tickCount);
	...
	private void inputTick() {
		...
		if ((Mouse.isButtonDown(0)) && hitResult != null && (System.currentTimeMillis() - lastClickTime > waitTime)) {
			...
			if (oldTile != Tile.air) {
				...
				if (player.destroyTile(oldTile, hitResult.x, hitResult.y, hitResult.z, particleEngine)) {
					if (oldTile == Tile.grass)
						oldTile = Tile.soil;
		...
		while (Mouse.next() && this.hitResult != null) {
			...
			if ((Mouse.getEventButton() == 1) && (Mouse.getEventButtonState())) {
				...
				if (isFree(Tile.getAABB(x, y, z)) && selector.getTileCount(selector.getPoint()) > 0) {
					if (!(selector.getSelectTile() instanceof LiquidTile && y > 55)) {
	...
	private void render(float a) {
		...
		levelRenderer.renderEntities(player, frustum, a);
		...
		guiRenderer.font.print("Craft " + VERSION, 0, 0, RGBAColor.white, 0, 1.0F);
		guiRenderer.font.print("FullScreen:" + FullScreen, 0, 15, RGBAColor.white, 0, 1.0F);
		guiRenderer.font.print(Chunk.updates + " Chunks updated, totalUpdates:" + Chunk.totalUpdates + ", tickCount:" + lastTickCount, 0, 30, RGBAColor.white, 0, 1.0F);
		guiRenderer.font.print("Player:X=" + Math.floor(player.x) + "  Y=" + Math.floor(player.y) + "  Z=" + Math.floor(player.z), 0, 45, RGBAColor.white, 0, 1.0F);
		guiRenderer.font.print("Mode:" + player.mode.name(), 0, 60, RGBAColor.white, 0, 1.0F);
		guiRenderer.font.print("Memory Used:" + new DecimalFormat("####.00").format((Runtime.getRuntime().totalMemory()) / 1000000.0) + "MB", 0, 75, RGBAColor.white, 0, 1.0F);
		guiRenderer.font.print("Fps:" + lastFps, 0, 90, RGBAColor.white, 0, 1.0F);
		Display.update();
	}
