...
public class Craft implements Runnable {
				...
				if (oldTile != Tile.air) {
					...
					if (player.destroyTile(oldTile, hitResult.x, hitResult.y, hitResult.z, particleEngine)) {
						...
						if (oldTile == Tile.grass)
							oldTile = Tile.soil;
						if (oldTile == Tile.stone)
							oldTile = Tile.cobblestone;
			...
			while (Mouse.next() && this.hitResult != null) {
				...
				if ((Mouse.getEventButton() == 1) && (Mouse.getEventButtonState())) {
					...
					if (isFree(Tile.getAABB(x, y, z)) && selector.getTileCount(selector.getPoint()) > 0) {
						if (!(selector.getSelectTile() instanceof LiquidTile && y > 60)) {
	...
	private void render(float a) {
		...
		guiRenderer.font.print(RenderChunk.updates + " Chunks updated, totalUpdates:" + RenderChunk.totalUpdates + ", tickCount:" + lastTickCount, 0, 30, RGBAColor.white, 0, 1.0F);
	...
	private void setupCamera(float a) {
		...
		GL11.glLoadIdentity();
		GLU.gluPerspective(70.0F, (float) width / height, 0.1F, 500.0F);
