...
public class Craft implements Runnable{
	...
	private static final String FILENAME = "date\\save\\level.dat";
	...
	public void init() throws LWJGLException {
		...
		level = new Level(128, 128, 256, FILENAME);
	...
	public void inputTick() {
		...
		if ((Mouse.isButtonDown(0)) && hitResult != null && (System.currentTimeMillis() - lastClickTime > 50)) {
		...
		while (Mouse.next() && this.hitResult != null) {
			...
			if ((Mouse.getEventButton() == 1) && (Mouse.getEventButtonState())) {
				...
				if(!player.aabb.intersects(Tile.getAABB(x, y, z)) && selector.getTileCount(selector.getPoint()) > 0) {
	...
	private void setupFog(int i) {
		if (i == 0) {
			...
			float br = 0.5F;
			GL11.glLightModel(GL11.GL_LIGHT_MODEL_AMBIENT, getBuffer(br, br, br, 0.3F));
		} else if (i == 1) {
			GL11.glFogi(GL11.GL_FOG_MODE, GL11.GL_VIEWPORT_BIT);
			...
			//float br = 0.8F;
			//GL11.glLightModel(GL11.GL_LIGHT_MODEL_AMBIENT, getBuffer(br, br, br, 1.0F));
		}
	}
