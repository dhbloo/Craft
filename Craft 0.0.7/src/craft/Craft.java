...
import craft.level.tile.ClamLiquidTile;
...
public class Craft implements Runnable{
	...
	private Timer smoothTimer = new Timer(60);
	private Level level;
	...
	public void init() throws LWJGLException {
		...
		level = new Level(256, 256, 128, FILENAME);
	...
	public void run() {
		...
		try {
			while (!Display.isCloseRequested()) {
				...
				inputTick();
				smoothTimer.advanceTime();
				for(int i = 0; i < smoothTimer.ticks; i++)
	...
	public void inputTick() {
		...
		if (player.mode == PlayerMode.Normal) {
			if ((Mouse.isButtonDown(0)) && hitResult != null && (System.currentTimeMillis() - lastClickTime > 25)) {
				...
				if (oldTile != Tile.air) {
					if (oldTile instanceof ClamLiquidTile)
		...
		while (Mouse.next() && this.hitResult != null) {
			...
			if ((Mouse.getEventButton() == 1) && (Mouse.getEventButtonState())) {
				...
				if(isFree(Tile.getAABB(x, y, z)) && selector.getTileCount(selector.getPoint()) > 0) {
					if(!(selector.getSelectTile() instanceof LiquidTile && y > 52)) {
	...
	public void render(float a) {
		...
		setupCamera(a);
		levelRenderer.cull(player, Frustum.getFrustum());
