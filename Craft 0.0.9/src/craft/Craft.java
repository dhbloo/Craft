...
public class Craft implements Runnable{
	...
	public void render(float a) {
		...
		levelRenderer.renderEntityTiles(frustum, a);
		
		particleEngine.render(player, a);
		
		for (int i = 0; i < humans.size(); i++) {
			Human human = (Human) humans.get(i);
			if (frustum.isVisible(human.aabb) && human.distanceToSqr(player) < 16384) {
				((Human) humans.get(i)).render(a);
			}
		}
