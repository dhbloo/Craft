...
public class Level {
	...
	public static final int TICK_SPEED = 4;
	...
	public Level(int length, int width, int height) {
		...
		this.blocks = new LevelGen(length, height, width).generateNewLevel();
	}
	...
	public void tick() {
		...
		int ticks = unprocessed / ((length + height + width) * 16 / TICK_SPEED);
		...
		for (int i = 0; i < ticks; i++) {
			...
			if (getBlock(x, y, z) != TileConst.AIR.value)
