...
public class EntityTileSand extends Entity {
	public static final float FALL_SPEED = 0.2F;

	public EntityTileSand(Level level, int x, int y, int z) {
		super(level);
		setSize(0.98F, 0.98F);
	...
	public void tick() {
		...
		move(-FALL_SPEED);
		if (onGround) {
			if (level.getBlock(xa, ya, za) != Tile.sand.id)
				level.setBlock(xa, ya, za, Tile.sand.id);
			remove();
		}
		if (y < -128.0F)
			remove();
	...
	public void render(Tesselator t, float a) {
		int xt = Tile.sand.tex % 16 * 16;
		int yt = Tile.sand.tex / 16 * 16;
		float u0 = xt / 256.0F;
