...
public class LevelRenderer implements LevelRendererListener {
	...
	public LevelRenderer(Level level) {
		...
		xChunks = level.length >> CHUNK_SHIFT_COUNT;
		yChunks = level.height >> CHUNK_SHIFT_COUNT;
		zChunks = level.width >> CHUNK_SHIFT_COUNT;
	...
	private ArrayList<RenderChunk> getNearByRenderChunks(Player player) {
		...
		for (int x = 0; x < xChunks; x++)
			for (int y = 0; y < yChunks; y++)
				for (int z = 0; z < zChunks; z++) {
					RenderChunk chunk = renderChunks[x][y][z];
					if (level.isChunkLoaded(chunk.chunkx, chunk.chunkz) && chunk.distanceToSqr(player) < GameSetting.instance.getFloat(DisplayOption.RenderDistance) * 1.5F) {
						if (dirtyRenderChunks == null)
							dirtyRenderChunks = new ArrayList<RenderChunk>(xChunks * yChunks * zChunks);
						dirtyRenderChunks.add(chunk);
					}
				...
				for (int z = z0; z < z1; z++) {
					...
					if (level.getBlock(x, y, z) != Tile.air.id) {
						...
						for(Face face : Face.values()) {
							...
							t.begin();
							if (Tile.tiles[level.getBlock(x, y, z)].mayPick())
	...
	public void complieSurroundingGroundList() {
		...
		Tesselator t = Tesselator.instance;
		float y = Level.GROUND_LEVEL;
		int s = Level.GROUND_WIDTH;
		if (s > level.length)	s = level.length;
		if (s > level.width)	s = level.width;
		/**边界地表宽度系数*/
		int d = 3;
		t.begin();
		for (int xx = -s * d; xx < level.length + s * d; xx += s) {
			for (int zz = -s * d; zz < level.width + s * d; zz += s) {
				float yy = y;
				if ((xx >= level.x0) && (zz >= level.z0) && (xx < level.x1) && (zz < level.z1))
					continue;
		...
		t.begin();
		for (int xx = level.x0; xx < level.x1; xx += s) {
			t.vertexUV(xx + 0, 0.0F, level.z0, 0.0F, 0.0F);
			t.vertexUV(xx + s, 0.0F, level.z0, s, 0.0F);
			t.vertexUV(xx + s, y, level.z0, s, y);
			t.vertexUV(xx + 0, y, level.z0, 0.0F, y);

			t.vertexUV(xx + 0, y, level.z1 + 1.0F, 0.0F, y);
			t.vertexUV(xx + s, y, level.z1 + 1.0F, s, y);
			t.vertexUV(xx + s, 0.0F, level.z1 + 1.0F, s, 0.0F);
			t.vertexUV(xx + 0, 0.0F, level.z1 + 1.0F, 0.0F, 0.0F);
		}
		GL11.glColor3f(0.6F, 0.6F, 0.6F);
		for (int zz = level.z0; zz <= level.z1; zz += s) {
			t.vertexUV(level.x0, y, zz + 0, 0.0F, 0.0F);
			t.vertexUV(level.x0, y, zz + s, s, 0.0F);
			t.vertexUV(level.x0, 0.0F, zz + s, s, y);
			t.vertexUV(level.x0, 0.0F, zz + 0, 0.0F, y);

			t.vertexUV(level.x1 + 1.0F, 0.0F, zz + 0, 0.0F, y);
			t.vertexUV(level.x1 + 1.0F, 0.0F, zz + s, s, y);
			t.vertexUV(level.x1 + 1.0F, y, zz + s, s, 0.0F);
			t.vertexUV(level.x1 + 1.0F, y, zz + 0, 0.0F, 0.0F);
		}
		t.end();
		GL11.glEndList();
	}
	...
	public void complieSurroundingWaterList() {
		...
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		Tesselator t = Tesselator.instance;
		float y = Level.GROUND_LEVEL + 2.0F;
		int s = Level.GROUND_WIDTH;
		if (s > level.length)	s = level.length;
		if (s > level.width)	s = level.width;
		/**边界地表宽度系数*/
