...
public class RenderChunk {
	...
	private static Tesselator t = Tesselator.instance;
 	...
 	public RenderChunk(Level level, int x0, int y0, int z0, int x1, int y1, int z1) {
		this.level = level;
		this.x0 = x0;
		this.y0 = y0;
		this.z0 = z0;
		...
		this.chunkx = (x0 + level.length / 2) / 16;
		this.chunkz = (z0 + level.width / 2) / 16;
	}
			...
			for (int y = y0; y <= y1; y++)
				for (int z = z0; z <= z1; z++) {
					Tile tile = Tile.tiles[level.getBlock(x, y, z)];
						//Tile tile = Tile.tiles[level.chunks[chunkx][chunkz].getBlock(x - x0, y, z - z0)];
					if (tile != Tile.air) {
						tile.render(t, level, x, y, z, layer);
						tiles++;
					}
				}
		t.end();
