...
public class AABB {
	...
	public float clipXCollide(AABB c, float xa) {
		...
		if ((xa > 0.0F) && (c.x1 <= this.x0)) {
			float max = this.x0 - c.x1;
			if (max < xa)
				xa = max;
		}
		if ((xa < 0.0F) && (c.x0 >= this.x1)) {
			float max = this.x1 - c.x0;
			if (max > xa)
				xa = max;
		}
	...
	public float clipYCollide(AABB c, float ya) {
		...
		if ((ya > 0.0F) && (c.y1 <= this.y0)) {
			float max = this.y0 - c.y1;
			if (max < ya)
				ya = max;
		}
		if ((ya < 0.0F) && (c.y0 >= this.y1)) {
			float max = this.y1 - c.y0;
			if (max > ya)
				ya = max;
		}
	...
	public float clipZCollide(AABB c, float za) {
		...
		if ((za > 0.0F) && (c.z1 <= this.z0)) {
			float max = this.z0 - c.z1;
			if (max < za)
				za = max;
		}
		if ((za < 0.0F) && (c.z0 >= this.z1)) {
			float max = this.z1 - c.z0;
			if (max > za)
				za = max;
		}
	...
	/** 不生成新AABB，直接设定这个AABB */
	public void setBounds(float x0, float y0, float z0, float x1, float y1, float z1) {
		this.x0 = x0;
		this.y0 = y0;
		this.z0 = z0;
		this.x1 = x1;
