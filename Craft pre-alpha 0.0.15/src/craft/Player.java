...
public class Player extends Entity{
	...
	public boolean destroyTile(Tile tile, int x, int y, int z, PickupEngine pickupEngine) {
		...
		destoryTime++;
		if (tile.hardness == -1) return false;
		if(destoryTime < tile.hardness && mode == PlayerMode.Normal) {
			tile.destroying(world, dx, dy, dz);
			return false;
		}
		destoryTime = -PLAYER_DESTORY_WAIT_TIME;
		tile.destroyed(world, dx, dy, dz, pickupEngine);
		return true;
	}
	...
	public void resetPos() {
		int y = spawnY;
		if (!world.isAirBlock(spawnX, y, spawnZ)) {
			y = world.maxHeight + 2;
			while (world.isAirBlock(spawnX, y - 1, spawnZ)) y--;
			y += 2;
		}
	...
	public void setRandomSpawnPos() {
		spawnX = (int) (world.x0 + Math.random() * world.length);
		spawnY = world.maxHeight + 2;
		spawnZ = (int) (world.z0 + Math.random() * world.width);
		while (world.isAirBlock(spawnX, spawnY - 1, spawnZ)) spawnY--;
		spawnY += 2;
	}
